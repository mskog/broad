...
describe Services::Trakt::Token do
  ...
  describe "#refresh" do
    Given do
      stub_request(:post, "https://api-v2launch.trakt.tv/oauth/token").with(body: {refresh_token: refresh_token, grant_type: "refresh_token"}.to_json).to_return(body: File.new("spec/fixtures/trakt/refresh_token.json").read)
    end
