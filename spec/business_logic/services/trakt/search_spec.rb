...
describe Services::Trakt::Search do
  ...
  describe "#movies" do
    context "simple query" do
      Given do
        stub_request(:get, "https://api-v2launch.trakt.tv/search?query=deadpool&type=movie").to_return(body: JSON.parse(File.new("spec/fixtures/trakt/search/movie_deadpool.json").read))
      end
      ...
      And{expect(first_result.ids.trakt).to eq 190_430}
      And{expect(first_result.ids.slug).to eq "deadpool-2016"}
      And{expect(first_result.ids.tmdb).to eq 293_660}
    ...
    context "with year" do
      Given do
        stub_request(:get, "https://api-v2launch.trakt.tv/search?query=deadpool&type=movie&year=#{year}").to_return(body: JSON.parse(File.new("spec/fixtures/trakt/search/movie_deadpool.json").read))
      end
  ...
  describe "#shows" do
    context "simple query" do
      ...
      And{expect(first_result.ids.trakt).to eq 59_660}
      And{expect(first_result.ids.slug).to eq "better-call-saul"}
      And{expect(first_result.ids.tvdb).to eq 273_181}
      And{expect(first_result.ids.imdb).to eq "tt3032476"}
      And{expect(first_result.ids.tmdb).to eq 60_059}
      And{expect(first_result.ids.tvrage).to eq 37_780}
  ...
  describe "#id" do
    ...
    context "show by imdb id" do
      ...
      Given(:id){"tt1431045"}
      Given(:id_type){"imdb"}
      When(:result){subject.id(id, id_type: id_type, type: :show)}
      Then{expect(result.first.title).to eq "Better Call Saul"}
      And{expect(result.first.ids.tvrage).to eq 37_780}
