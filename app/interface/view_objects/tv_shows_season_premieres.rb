module ViewObjects
  class TvShowsSeasonPremieres
    ...
    def episodes
      @episodes ||= trakt_calendar.show_premieres.compact
    end
    ...
    def cache_key
      ['viewobjects', 'tv_shows_season_premieres', @cache_key_prefix, @from_date.try(:to_time).try(:to_i), @days].compact.join('-')
    end
